...
CREATE OR REPLACE FUNCTION public.find_or_create_conversation(p_empresa_id uuid, p_phone text, p_client_name text, p_client_id uuid, p_last_message text, p_last_message_at timestamp with time zone)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- First try with FOR UPDATE (wait for lock, don't skip)
