      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {forms.map(form => {
            ...
            return (
              <div key={form.id} className={`border rounded-lg p-3 space-y-2 ${form.status === 'submitted' ? 'bg-emerald-50 border-emerald-200' : form.status === 'sent' ? 'bg-orange-50 border-orange-200' : form.status === 'deleted' ? 'bg-red-50 border-red-200' : ''}`}>
                {showBanner && (
