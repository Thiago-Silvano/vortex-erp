...
export default function DS160Step13({ data, onChange, errors }: DS160StepProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {data.clan_tribo && <TextField label="Nome do clã/tribo" value={data.clan_tribo_nome} onChange={v => onChange('clan_tribo_nome', v)} />}
      ...
      {data.habilidades_especiais && <AreaField label="Explicação" value={data.habilidades_especiais_explicacao} onChange={v => onChange('habilidades_especiais_explicacao', v)} />}
      ...
      {data.servico_militar && (
        <div className="space-y-3 rounded-lg border border-slate-200 bg-slate-50/60 p-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <CountryField label="País" value={data.militar_pais || 'Brasil'} onChange={v => onChange('militar_pais', v)} />
            <TextField label="Ramo" value={data.militar_ramo} onChange={v => onChange('militar_ramo', v)} />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <DateField label="Data de início" value={data.militar_data_inicio} onChange={v => onChange('militar_data_inicio', v)} />
      ...
      {data.paramilitar && <AreaField label="Explicação" value={data.paramilitar_explicacao} onChange={v => onChange('paramilitar_explicacao', v)} />}
    </div>
  );
}
