    ...
    <div className="space-y-6">
      ...
      {data.possui_contato_eua === 'Tenho contato' && (
        <div className="space-y-4">
          <div><Label>Nome Completo do Contato</Label><Input className={errClass(errors?.contato_eua_nome)} value={data.contato_eua_nome || ''} onChange={e => onChange('contato_eua_nome', e.target.value)} /><FieldError msg={errors?.contato_eua_nome} /></div>
          ...
          <div><Label>Relação com o Contato</Label><Input className={errClass(errors?.contato_eua_relacao)} value={data.contato_eua_relacao || ''} onChange={e => onChange('contato_eua_relacao', e.target.value)} /><FieldError msg={errors?.contato_eua_relacao} /></div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><Label>Telefone do Contato</Label><Input className={errClass(errors?.contato_eua_telefone)} value={data.contato_eua_telefone || ''} onChange={e => onChange('contato_eua_telefone', e.target.value)} /><FieldError msg={errors?.contato_eua_telefone} /></div>
