...
export function generateVoucherPdf(data: VoucherPdfData, existingDoc?: jsPDF) {
  ...
  // Ordenar por data de hospedagem (check-in para hotéis, date para serviços)
  pages.sort((a, b) => {
    const dateA = a.type === "hotel" ? a.hotel?.checkIn : a.service?.date;
    const dateB = b.type === "hotel" ? b.hotel?.checkIn : b.service?.date;
