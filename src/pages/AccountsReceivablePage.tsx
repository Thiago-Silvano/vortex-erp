...
export default function AccountsReceivablePage() {
  ...
  const fetch_ = async () => {
    ...
    setItems((data as unknown as Receivable[]).filter(r => !r.sale_id || !draftIds.includes(r.sale_id)));
  };
  ...
  const indicatorCards = [
    ...
    { label: 'A Receber', value: indicators.pending, icon: DollarSign, color: 'text-primary', bg: 'bg-primary/10' },
    { label: 'Recebidos', value: indicators.received, icon: CheckCircle, color: 'text-emerald-600', bg: 'bg-emerald-500/10' },
  ];
      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1 bg-card border rounded-md p-1 shadow-sm">
            <Button size="sm" variant="ghost" className="gap-1.5" onClick={() => navigate(`/financial/payable${window.location.search}`)}>
