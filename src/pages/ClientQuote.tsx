...
import { AgencySettings, SERVICE_TYPE_CONFIG, ServiceItem, FlightLeg } from '@/types/quote';
import { MessageCircle, Plane, Hotel, Car, Shield, Ticket, FileText, MapPin, Calendar, Users, Moon, ChevronDown } from 'lucide-react';
            ...
            {item.flightLegs.map((leg, idx) => (
              <div key={idx} className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                <div className="flex items-center gap-2 mb-3">
                  <span className="bg-[#1a2744] text-white text-xs font-bold px-2 py-1 rounded">Trecho {idx + 1}</span>
                </div>
        ...
        <div className="space-y-2 text-sm">
          {item.supplier && (
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-[#1a2744]">{item.supplier}</span>
            </div>
          )}
          {item.location && (
            <div className="flex items-center gap-2">
      ...
      {images.length > 1 && (
        <div className="px-6 pb-6 flex gap-2 overflow-x-auto">
          {images.slice(1).map((img, idx) => (
            <img key={idx} src={img} alt="" className="h-20 w-28 rounded-lg object-cover flex-shrink-0 border border-gray-100" />
          ))}
        </div>
      )}
    </div>
      ...
      <section className="relative -mt-1 bg-white rounded-t-3xl pt-12 pb-8 px-6 md:px-12 lg:px-20 shadow-xl">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-6 max-w-4xl mx-auto">
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-gray-500">a {formatDate(quote.trip.returnDate)}</p>
          </div>
          <div className="text-center">
            <div className="w-12 h-12 bg-[#1a2744] rounded-full flex items-center justify-center mx-auto mb-3">
